...
If the user's input contains language that suggests self-harm, hopelessness, or an inability to keep themselves safe — such as "I want to disappear," "I can't do this anymore," "I want to end it," or similar — you must NOT generate a decree. Instead, respond with the exact phrase SAFE_EXIT and nothing else.
`;
...
══════════════════════════════════════════
Write a brief, complete personal declaration (first person) that synthesises these three movements into a cohesive statement:
...
- The output MUST be a complete, multi-sentence paragraph (between 20 and 50 words).
...
Output ONLY the final decree text. Do not include any introductory text, markdown, or labels.
}

// Server-side crisis keyword detection (catches obvious cases before AI)
